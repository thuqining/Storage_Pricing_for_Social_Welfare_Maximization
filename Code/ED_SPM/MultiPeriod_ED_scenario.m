%% initial test
% price generation
function [energy_price,ESenergycap]=MultiPeriod_ED_scenario(a,b,c,d,e,f,g)
%% inputs
...
load_d1(:,i)= normrnd(load_c(i)+mu(i), sigma(i));
...
for j=1:T    
cons=[cons, sum(Pg(:,j),1)+sum(Pdis(:,j),1)-sum(Pch(:,j),1)==load_d1(:,j)];
end
