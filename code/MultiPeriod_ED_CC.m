...
function [bid_bound_c,bid_bound_d]=MultiPeriod_ED_CC(a,b,c,d,e,f,g,h)
%% inputs
% a=0.3;
% b=0.05;
% c=1.5; %sigma, original forecast error is too small
% d=10; %day number
...
% end
% if epsilon<1/6
% load_d1(:,i)= mu(i)-sqrt((4-9*epsilon)/(9*epsilon))*sigma(i);
% else 
%     load_d1(:,i)= mu(i)-((3-3*epsilon)/(1+3*epsilon))*sigma(i);
% end
%robust approximation2 
%load_d1(:,i)= mu(i)-sqrt((1-epsilon)/(epsilon))*sigma(i);
%Empirical
%load_d1(:,i)= quantile(B(:,i)-mu(i), epsilon)*c+mu(i);%
%versatile
...
% end
% if epsilon<1/6
% load_d2(:,i)= mu(i)+sqrt((4-9*epsilon)/(9*epsilon))*sigma(i);
% else 
%     load_d2(:,i)= mu(i)+((3-3*epsilon)/(1+3*epsilon))*sigma(i);
% end
%robust approximation2 
%load_d2(:,i)= mu(i)+sqrt((1-epsilon)/(epsilon))*sigma(i);
%Empirical
%load_d2(:,i)= quantile(B(:,i)-mu(i), 1-epsilon)*c+mu(i);
%versatile
